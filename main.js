...
const taskManager = (() => {
  ...
  const updateAllArrays = function () {
    ...
    for (let i = 0; i < inboxTasksArray.length; i++) {
      ...
      if (
        inboxTasksArray[i].formattedDueDate ===
        date_fns__WEBPACK_IMPORTED_MODULE_1__.format(new Date(), "dd MMMM yyyy")
      ) {
        todayTasksArray.push(inboxTasksArray[i]);
      }

      if (
        new Date(inboxTasksArray[i].formattedDueDate).getDate() <=
          seventhDay.getDate() &&
        new Date(inboxTasksArray[i].formattedDueDate).getDate() >=
          new Date().getDate()
      ) {
  ...
  const getInboxTasks = () =>
    inboxTasksArray.filter((task) => task.projectIndex == 0);
  ...
  const getTaskEditDetails = function (taskIndex) {
    ...
    const formattedDueDate =
      taskObject.formattedDueDate !== ""
        ? date_fns__WEBPACK_IMPORTED_MODULE_1__.format(taskObject.formattedDueDate, "yyyy-MM-dd")
        : "";
  ...
  const getProjectTasksArray = (projectIndex) =>
    inboxTasksArray.filter((task) => task.projectIndex == projectIndex);
  ...
  const toggleTaskCompleteStatus = function (taskElement) {
    ...
    inboxTasksArray[taskObjectIndex].complete = !isComplete;

    _localStorage__WEBPACK_IMPORTED_MODULE_0__.localStorageManager.saveTasksArrayToLocalStorage(inboxTasksArray);
  };

  const formateDate = (dueDate) =>
    dueDate === "" ? "" : date_fns__WEBPACK_IMPORTED_MODULE_1__.format(dueDate, "dd MMMM yyyy");

  const deleteTasksByProjectIndex = (projectIndex) =>
    inboxTasksArray.forEach((task, index) => {
  ...
  const getTaskProjectIndex = (taskIndex) =>
    inboxTasksArray[taskIndex].projectIndex;

  const setTaskToEditMode = (taskIndex) =>
    (inboxTasksArray[taskIndex].edit = true);

  const getTaskObjectInEditMode = () =>
    inboxTasksArray.find((task) => task.edit);
  ...
  const updateIndoxTasksArray = (storedArray) =>
    (inboxTasksArray = storedArray);
  ...
  return {
    ...
    addTaskItemToArray,
    getTodayTasksArray,
    getThisWeekTasksArray,
    toggleTaskCompleteStatus,
    getCompleteTasksArray,
    getProjectTasksArray,
    getTaskDetails,
    deleteTasksByProjectIndex,
    getTaskEditDetails,
    getTaskProjectIndex,
    setTaskToEditMode,
    getTaskObjectInEditMode,
    updateTaskDetails,
    deleteTaskByTaskIndex,
    getInboxTasksArray,
    updateIndoxTasksArray,
  };
})();
...
(function createEvents() {
  ...
  projectDialogCloseButton.addEventListener('click', () => {
    if (projectEditMode) {
      const projectObjectInEditMode =
        _project__WEBPACK_IMPORTED_MODULE_2__.projectManager.getProjectObjectInEditMode();
      projectObjectInEditMode.edit = false;
  ...
  projectForm.addEventListener('submit', (event) => {
    ...
    if (projectEditMode) {
      ...
      const projectTasksArray = _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.getProjectTasksArray(
    ...
    if (target.classList.contains('task-check-button')) {
      ...
      if (displayController.isProjectTab()) {
        ...
        const projectTasksArray =
          _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.getProjectTasksArray(projectIndex);
        projectIndex--;
        displayController.checkRenderingCondition(
          projectTasksArray,
          projectIndex,
  ...
  navBar.addEventListener('click', (event) => {
    ...
    if (target.classList.contains('project-edit-button')) {
      ...
      const projectName =
        _project__WEBPACK_IMPORTED_MODULE_2__.projectManager.getProjectNameByProjectIndex(projectIndex);
    ...
    if (projectElement) {
      ...
      const projectNameIndex =
        _project__WEBPACK_IMPORTED_MODULE_2__.projectManager.getProjectNameIndex(taskProjectIndex);

      const projectTasksArray =
        _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.getProjectTasksArray(taskProjectIndex);
  ...
  function createNewTaskObject(projectName, projectIndex) {
    ...
    return _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.createTaskItem(
      taskTitleValue,
...
(function defaultStart() {
  if (!localStorage.getItem('inboxTasksArray')) {
    ...
    const inboxTasksArray =
      _localStorage__WEBPACK_IMPORTED_MODULE_3__.localStorageManager.loadTasksArrayFromLocalStorage();
    const projectsArray =
      _localStorage__WEBPACK_IMPORTED_MODULE_3__.localStorageManager.loadProjectsArrayFromLocalStorage();
  ...
  function displayDefaultTasks() {
    ...
    const sampleProject = _project__WEBPACK_IMPORTED_MODULE_2__.projectManager.createProjectObject(
      'The Most Amazing Project',
    );
    const sampleTaskOne = _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.createTaskItem(
      'Feed Cat',
    ...
    const sampleTaskTwo = _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.createTaskItem(
      'Do Leetcode Ex',
    ...
    const sampleTaskThree = _task__WEBPACK_IMPORTED_MODULE_1__.taskManager.createTaskItem(
